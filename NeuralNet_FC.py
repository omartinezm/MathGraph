...
class NeuralNet_FC(Graph):
    ...
    def propagate(self,sLayer=0,edge=None,back=False,key=None):
        ...
        for i in range(self.npl[-1]):
            self.vertex["L"+str(len(self.npl)-1)+"_v"+str(i)].update_val(key,0)
        s=sum(self.npl)
        if back:
            ...
            for neu in tqdm(self.vertex,desc="Prop. on "+str(self.nlayers)+" layers"):
                ...
                if not "L"+str(len(self.npl)-1) in neu:
                    #print(neu)
                    self.vertex[neu].update_val(key,0)
    ...
    def predict(self,data=[],key=None):
        ...
        for vec in data:
            ...
            if len(vec)==self.npl[0]:
                ...
                r1=[]
                for i in range(self.npl[-1]):
                    r1.append(self.vertex["L"+str(len(self.npl)-1)+"_v"+str(i)].get_updater_val(key))
                res.append(r1)
        return res
    def __repr__(self):
